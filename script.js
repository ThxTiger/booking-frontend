...
let username = ""; 
let availableRooms = []; // 🆕 Store room data locally
...
function handleRoomChange() {
    ...
    if (room) {
        // Update the Room Info Card
        document.getElementById('roomInfoCard').classList.remove('d-none');
        document.getElementById('infoName').textContent = room.displayName;
        document.getElementById('infoFiliale').textContent = room.department; // e.g. "Axians"
        document.getElementById('infoFloor').textContent = room.floor;       // e.g. "Floor 3"
...
// (Rest of the auth logic/timeline rendering is the same as previous...)
// Copy 'signIn', 'signOut', 'handleLoginSuccess', 'renderTimeline', 'initModalTimes' from previous version
        ...
        schedule.scheduleItems.forEach(item => {
            if (item.status === 'busy') {
                ...
                if (leftPct < 100 && (leftPct + widthPct) > 0) {
                    headerHtml += `<div class="event-block" style="left:${Math.max(0, leftPct)}%; width:${Math.min(widthPct, 100 - Math.max(0, leftPct))}%;" title="${item.subject}"><span>🚫 Busy</span></div>`;
                }
            }
        });
    }
    timelineContainer.innerHTML = headerHtml + `</div>`;
}
